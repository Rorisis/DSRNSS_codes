...
@MODEL_REGISTRY
class DomainAdaptationSegmentationModelRefineNet(pl.LightningModule):
    def __init__(self,
                 optimizer_init: dict,
    ...
    def training_step(self, batch, batch_idx):
        ...
        feats_src,logits_src = self.backbone(images_src)
        ...
        with torch.no_grad():
            ...
            if self.use_refign and not adapt_to_ref:
                ...
                _,m_logits = self.m_backbone(m_input)
                ...
                _ , m_logits_trg = self.m_backbone(images_trg)
        ...
        mixed_feats, mixed_pred = self.backbone(mixed_img)
        if self.use_hrda:
    ...
    def ema_parameters(self):
        for m in filter(None, [self.m_backbone, self.m_hrda_scale_attention]):
            for p in m.parameters():
                yield p

    def live_parameters(self):
        for m in filter(None, [self.backbone, self.hrda_scale_attention]):
            for p in m.parameters():
                yield p
